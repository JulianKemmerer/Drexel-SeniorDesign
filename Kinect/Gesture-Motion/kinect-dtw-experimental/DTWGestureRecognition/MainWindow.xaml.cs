...
namespace DTWGestureRecognition
{
    ...
    public partial class MainWindow
    {
        ...
        /// <summary>
        /// How many seconds to store in the video buffer
        /// </summary>
        private const int BufferSeconds = 1;

        /// <summary>
        /// How many skeleton frames to store in the _video buffer, 30 fps * # seconds
        /// </summary>
        private const int BufferSize = 30 * BufferSeconds;
        ...
        private Point GetDisplayPosition(Joint joint)
        {
            ...
            if (colorX > 640 || colorY > 480)
            {
                Debug.WriteLine(colorX + " " + colorY);
                //return new Point();
            }
        ...
        {
            //This is also called when capture has ended
            //Switch off visual indicator
        ...
        */
        ...
        private void selectAudioBtn_Click(object sender, RoutedEventArgs e)
        {
            // Create OpenFileDialog
            Microsoft.Win32.OpenFileDialog dlg = new Microsoft.Win32.OpenFileDialog();
            dlg.ShowDialog();
            audioFileName = dlg.FileName;
            fileNameTextBox.Text = audioFileName;
        }
        ...
        private void doAudioAction(string action)
        {
            if (isPlaying)
            {

                if (action.Contains("Right hand swipe right") || action.Contains("Left hand swipe right"))
                {
                    mp.Balance = 1;
                }
                else if (action.Contains("Right hand swipe left") || action.Contains("Left hand swipe left"))
                {
                    mp.Balance = -1;
                }
                else if (action.Contains("Right hand pull down") || action.Contains("Left hand pull down"))
                {
                    mp.Volume -= .2;
                }
                else if (action.Contains("Left hand push up") || action.Contains("Right hand push up"))
                {
                    mp.Volume += .2;
                }
                else
                {
                    Debug.WriteLine("Did not recognize action: " + action);
                }
            }
