...
//ExCITe Macbook
//cd "C:\Users\ExCITe\Dropbox\Drexel\Classes\Junior Year\Senior Design\CentralProcessing\GUI\CSharp\gui_dll\bin\Debug" && RegAsm.exe gui_dll.dll /tlb:gui_dll.tlb /codebase && RegAsm.exe gui_form.exe /tlb:gui_form.tlb /codebase
*/
...
class app : public ofBaseApp{
	public:
		...
		//Function filled with lovely sound.
		...
		int findOutputIndex(string to_find,int out, int in);
		void clean_sound();
		//Sound parameters
		...
		//Global list of output ptrs for each channel
		vector<float *> fl_samples;
		vector<float *> fr_samples;
		vector<float *> sl_samples;
		vector<float *> sr_samples;
		float * output_buffer; //To compare for refresh of channels
		...
		string current_gui_selected_skeleton_stem;
		...
		void drawJoint(Joint* j, ofPoint multiplier, ofPoint constant, Bone * b, Skeleton * s);
		void drawJointText(Joint* j, ofPoint multiplier, ofPoint constant, ofPoint realpt, Bone * b, Skeleton * s);
		int numSkeletonOSCArgs;
		Skeleton * findSkeleton(int index, int src_kinect);
		void special_skeleton_copy(Skeleton * to, Skeleton * from);

		//Audio effects section?
		void doPanEffect(float panX, float panY);
		vector<Chirp *> getChirpsFromString(string str);
		void init_add_chirp(float dur_ms,float freq_start,float freq_end,float rep_period, float volume, float start_offset_ms, string speaker_id);
		//Global list of chirps
		vector<Chirp*> chirps;
		//Global list of single wave file players
		vector<SingleWAVFileAudioPlayer*> wavFilePlayers;
		vector<SingleWAVFileAudioPlayer *> findWAVFilePlayersFromTrackAndJoint(Joint * j, string filename);
		//Global list of audio input audio objects
		vector<AudioInput *> audioInputs;
		AudioInput leftAudioInput;
		AudioInput rightAudioInput;
		//Audio input buffers
		float * lin_buffer;
		float * rin_buffer;
		//Skeleton settings from gui
		bool skeletonSettings(string name, string value);
		string getFileNameFromTrackMappingString(string mapping, string prefix);
		void replaceAll(std::string& str, const std::string& from, const std::string& to);
		float getPanXFromJoint(Joint * j);
		float getPanYFromJoint(Joint * j);
		float getVolFromJoint(Joint * j);
		void addOrUpdateAudioEffect(Skeleton * s, Bone * b, Joint * j);
		SingleWAVFileAudioPlayer * init_add_start_wavfileplayer(string filename);
		...
		static const string app::Setting_HandLeftMap;
		static const string app::Setting_WristRightMap;
		static const string app::Setting_WristLeftMap;
		static const string app::Setting_ElbowRightMap;
		static const string app::Setting_ElbowLeftMap;
		static const string app::Setting_ShoulderRightMap;
		static const string app::Setting_ShoulderLeftMap;
		static const string Setting_GlobalVolume;
		static const string Setting_GlobalPan ;
		//Setting checkers
